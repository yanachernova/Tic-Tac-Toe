...
class TicTac extends React.Component {
  constructor(props) {
    ...
    this.selctPlayer = this.selctPlayer.bind(this); //for each arrow function in a class function 
  ...
  handleInput(y) {
    ...
    if (y.target.id === "input1") {
      this.setState({ player1: v.value }) 
    } 
    else {
      this.setState({ player2: v.value })
    }
  ...
  render() {
    return (
      <div className="container text-center p-2" style={styleTop}>
        <h1 style={{ color: 'white' }}>Tic Tac Toe in React.js </h1>
        ...
        <div className={this.state.visual === 3 ? this.state.visible : this.state.invisible}><Winner player={this.state.winner} /></div>
      </div>
